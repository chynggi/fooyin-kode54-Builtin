set(CPACK_PACKAGE_NAME "fooyin")
set(CPACK_PACKAGE_VENDOR "fooyin")
set(CPACK_PACKAGE_CONTACT "Luke Taylor <luket@pm.me>")
set(CPACK_PACKAGE_DESCRIPTION_SUMMARY "A customisable music player")
set(CPACK_PACKAGE_INSTALL_DIRECTORY ${CPACK_PACKAGE_NAME})
set(CPACK_VERBATIM_VARIABLES ON)
set(CPACK_PACKAGE_EXECUTABLES ${CPACK_PACKAGE_NAME} ${CPACK_PACKAGE_NAME})
set(CPACK_RESOURCE_FILE_LICENSE "${CMAKE_CURRENT_SOURCE_DIR}/COPYING")
set(CPACK_RESOURCE_FILE_README "${CMAKE_CURRENT_SOURCE_DIR}/README.md")
set(CPACK_PACKAGE_VERSION_MAJOR "${FOOYIN_VERSION_MAJOR}")
set(CPACK_PACKAGE_VERSION_MINOR "${FOOYIN_VERSION_MINOR}")
set(CPACK_PACKAGE_VERSION_PATCH "${FOOYIN_VERSION_PATCH}")
set(CPACK_PACKAGE_VERSION "${FOOYIN_VERSION}")
set(CPACK_PACKAGE_RELOCATABLE
    ON
    CACHE BOOL "Build relocatable package"
)
set(CPACK_PACKAGE_RELEASE 1)

set(FOOYIN_PACKAGE_ARCH "${CMAKE_SYSTEM_PROCESSOR}")
if(CMAKE_VS_PLATFORM_NAME)
    set(FOOYIN_PACKAGE_ARCH "${CMAKE_VS_PLATFORM_NAME}")
elseif(APPLE AND CMAKE_OSX_ARCHITECTURES)
    list(GET CMAKE_OSX_ARCHITECTURES 0 FOOYIN_PACKAGE_ARCH)
endif()

if(FOOYIN_PACKAGE_ARCH MATCHES "^(ARM64|arm64|AARCH64|aarch64)$")
    set(FOOYIN_PACKAGE_ARCH "arm64")
elseif(FOOYIN_PACKAGE_ARCH MATCHES "^(AMD64|amd64|X64|x64|x86_64|X86_64)$")
    if(APPLE)
        set(FOOYIN_PACKAGE_ARCH "x86_64")
    else()
        set(FOOYIN_PACKAGE_ARCH "x64")
    endif()
else()
    string(TOLOWER "${FOOYIN_PACKAGE_ARCH}" FOOYIN_PACKAGE_ARCH)
endif()

if(WIN32)
    set(CPACK_GENERATOR "NSIS" "ZIP")
    set(CPACK_FOOYIN_WINDOWS_PACKAGE_FILE_NAME "${CPACK_PACKAGE_NAME}-${CPACK_PACKAGE_VERSION}-windows-${FOOYIN_PACKAGE_ARCH}")
    set(CPACK_PACKAGE_FILE_NAME "${CPACK_FOOYIN_WINDOWS_PACKAGE_FILE_NAME}")
    set(CPACK_PACKAGE_ICON "${CMAKE_SOURCE_DIR}/data/icons\\\\fooyin.ico")
    set(CPACK_NSIS_MUI_ICON "${CMAKE_SOURCE_DIR}/data/icons\\\\fooyin.ico")
    set(CPACK_NSIS_URL_INFO_ABOUT "https://www.fooyin.org/")
    set(CPACK_NSIS_INSTALLED_ICON_NAME "${CPACK_PACKAGE_NAME}{CMAKE_EXECUTABLE_SUFFIX}")
    set(CPACK_NSIS_MUI_FINISHPAGE_RUN "${CPACK_PACKAGE_NAME}.exe")
    set(CPACK_COMPONENTS_ALL fooyin)

    set(CPACK_NSIS_EXECUTABLES_DIRECTORY "${BIN_INSTALL_DIR}")
    set(CPACK_PACKAGE_INSTALL_DIRECTORY "fooyin")
    set(CPACK_INSTALL_SCRIPTS "${CMAKE_SOURCE_DIR}/cmake/FooyinPostInstall.cmake")
elseif(APPLE)
    set(CPACK_GENERATOR "DragNDrop")
    set(CPACK_PACKAGE_FILE_NAME "${CPACK_PACKAGE_NAME}-${CPACK_PACKAGE_VERSION}-${FOOYIN_PACKAGE_ARCH}")
    set(CPACK_DMG_VOLUME_NAME "${CPACK_PACKAGE_NAME}")
    set(CPACK_DMG_FILESYSTEM "APFS")
    set(CPACK_DMG_SLA_USE_RESOURCE_FILE_LICENSE OFF)
    set(CPACK_DMG_BACKGROUND_IMAGE "${FOOYIN_DMG_BACKGROUND_IMAGE}")
    set(CPACK_DMG_DS_STORE_SETUP_SCRIPT "${FOOYIN_DMG_DS_STORE_SETUP_SCRIPT}")
    set(CPACK_COMMAND_HDIUTIL "${CMAKE_SOURCE_DIR}/cmake/macos-hdiutil-retry.sh")
else()
    set(CPACK_PACKAGE_ICON "${CMAKE_SOURCE_DIR}/data/icons/sc-fooyin.svg")
endif()

set(CPACK_SOURCE_IGNORE_FILES "\\\\.#;/#;.*~;\\\\.o$")
list(APPEND CPACK_SOURCE_IGNORE_FILES "/\\\\.git/")
list(APPEND CPACK_SOURCE_IGNORE_FILES "/\\\\.github/")
list(APPEND CPACK_SOURCE_IGNORE_FILES "/build/")
list(APPEND CPACK_SOURCE_IGNORE_FILES "${CMAKE_CURRENT_BINARY_DIR}/")

set(CPACK_DEBIAN_PACKAGE_MAINTAINER "Luke Taylor <luket@pm.me>")
set(CPACK_DEBIAN_PACKAGE_HOMEPAGE ${CPACK_PROJECT_URL})
set(CPACK_DEBIAN_PACKAGE_SECTION "sound")
set(CPACK_DEBIAN_PACKAGE_PRIORITY "optional")
set(CPACK_DEBIAN_PACKAGE_DEPENDS
    "libc6,
    libstdc++6,
    libqt6sql6-sqlite (>= 6.4.0),
    libqt6svg6 (>= 6.4.0),
    qt6-image-formats-plugins (>= 6.4.0),
    qt6-qpa-plugins,
    zlib1g,
    libasound2,
    ffmpeg (>= 7:4.4),
    pipewire,
    libpulse0,
    libsdl2-2.0-0,
    libgme0,
    libopenmpt0,
    libarchive13,
    libsndfile1,
    libebur128-1,
    libsoundtouch1,
    libsoxr0,
    libmpg123-0t64,
    libvorbis0a,
    libvorbisfile3,
    libvorbisenc2,
    libspeex1,
    libflac14,
    libogg0"
)

set(CPACK_RPM_PACKAGE_LICENSE "GPL-3.0-or-later")
set(CPACK_RPM_PACKAGE_GROUP "Applications/Multimedia")
set(CPACK_RPM_PACKAGE_REQUIRES_POST "qt6-qtimageformats")

set(CPACK_PROJECT_CONFIG_FILE "${CMAKE_SOURCE_DIR}/cmake/FooyinPackageConfig.cmake")

include(CPack)
